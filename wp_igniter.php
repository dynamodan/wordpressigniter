...
if	( !is_admin() ) {
	...
	if( isset( $cookie_elements ) && ( $cookie_elements = wp_parse_auth_cookie( $_COOKIE[LOGGED_IN_COOKIE], 'logged_in' ) ) ) {
		...
		if ( defined( 'DOING_AJAX' ) || 'POST' == $_SERVER['REQUEST_METHOD'] ) $expired += HOUR_IN_SECONDS;
	...
	if ( !get_option( 'wp_igniter_ci_urihook' ) ) {
	...
	}// end if ( !get_option( 'wp_igniter_native_constants' ) )
	...
	class WP_Igniter {
		...
		public function main_content( $content ) {
			...
			if( $this->triggered ) {
				...
				if(  !get_option( 'wp_igniter_ci_path ') ) {
					$content .= "<br />You need to set the path to the CodeIgniter front controller."; 
					return $content; 
				} 
				...
				if( !file_exists( $ci_path )) { 
					return "Couldn't locate path ".$ci_path; 
				}
		...
		public function main_title( $content ) {
			...
			if( function_exists( 'get_instance' ) ) {
				...
				if( $this->triggered && isset( $ci->content['page_title'] ) ) { 
					$ci_title = $ci->content['page_title']; 
				}
				// return $ci_title;
			}
			
			// trigger by title
			if( !in_array( $content, $this->page_matches ) ) { 
				return $content; 
			}
			
			// only do this for the title within the post, not on the menu: (TODO: make a switch to alter this behaviour)
			if ( did_action( 'the_post' ) == 0 ) { 
				return $content; 
			}
			...
			if ( function_exists( 'get_instance' ) ) {
				...
				if( isset( $ci->content['page_title'] ) ) { 
					$ci_title = $ci->content['page_title']; 
				}
			}
			return $ci_title;
			
		} // end function main title
